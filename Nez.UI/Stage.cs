...
namespace Nez.UI
{
    public class Stage
    {
        ...
        private IGamepadFocusable _gamepadFocusElement;
        private readonly List<Element> _inputFocusListeners = new List<Element>();

        private bool _isGamepadFocusEnabled;
        private IKeyboardListener _keyboardFocusElement;
        private readonly float _keyRepeatTime = 0.2f;
        private ITimer _keyRepeatTimer;

        private Vector2 _lastMousePosition;
        private Keys[] _lastPressedKeys = EmptyKeys;
        private Element _mouseOverElement;
        private Keys _repeatKey;
        private readonly Dictionary<int, Element> _touchOverElement = new Dictionary<int, Element>();
        public Camera Camera;
        private bool _debugAll, _debugUnderMouse, _debugParentUnderMouse;
        private Table.TableDebug _debugTableUnderMouse = Table.TableDebug.None;
        ...
        {
            return _root.AddElement(element);
        }
        ...
        public void Render(Graphics.Graphics graphics, Camera camera)
        {
            ...
            this.Camera = camera;
            _root.Draw(graphics, 1f);

            if (Debug)
            {
                DrawDebug();
                _root.DebugRender(graphics);
            }
        }
        ...
        private void DrawDebug()
        {
            if (_debugUnderMouse || _debugParentUnderMouse || _debugTableUnderMouse != Table.TableDebug.None)
            {
                ...
                if (element == null)
                {
                    DisableDebug(_root, null);
                    return;
                }

                if (_debugParentUnderMouse && element.Parent != null)
                    element = element.Parent;

                if (_debugTableUnderMouse == Table.TableDebug.None)
                {
                ...
                else
                {
                    ...
                    ((Table) element).DebugTable(_debugTableUnderMouse);
                }

                if (_debugAll && element is Group)
                    ((Group) element).DebugAll();

                DisableDebug(_root, element);
            }
            else
            {
                if (_debugAll)
	    ...
	    private void DisableDebug(Element element, Element except)
        {
            if (element == except)
                return;
	    ...
	    /// </summary>
	    private void ClearKeyRepeatTimer()
        {
        ...
        {
            point = _root.ParentToLocalCoordinates(point);
            return _root.Hit(point);
        }
        ...
        private IGamepadFocusable FindNextGamepadFocusable(IGamepadFocusable relativeToFocusable, Direction direction)
        {
        ...
        {
            var eles = new List<T>();
            FindAllElementsOfType(_root.Children, eles);
            return eles;
        }


        private void FindAllElementsOfType<T>(List<Element> elements, List<T> foundElements) where T : class
        {
	    ...
	    private void UpdateInputTouch()
        {
            ...
            foreach (var touch in Input.Input.Touch.CurrentTouches)
            {
                ...
                var inputReleased = touch.State == TouchLocationState.Released ||
                ...
                if (Entity != null && !IsFullScreen)
                    inputPos = Input.Input.ScaledPosition(inputPos);
	    ...
	    private void UpdateInputPoint(Vector2 inputPos, bool inputPressed, bool inputReleased, bool inputMoved,
	    ...
	    private void UpdateInputDown(Vector2 inputPos, Element over)
        {
	    ...
	    private void UpdateInputMoved(Vector2 inputPos, Element over, Element lastOver)
        {
	    ...
	    private void UpdateInputReleased(Vector2 inputPos)
        {
            for (var i = _inputFocusListeners.Count - 1; i >= 0; i--)
        ...
        {
            if (this._debugUnderMouse == debugUnderMouse)
                return;

            this._debugUnderMouse = debugUnderMouse;
            if (debugUnderMouse)
                Debug = true;
            else
                _root.SetDebug(false, true);
	    ...
	    /// <summary>
	    ///     If true, debug is enabled only for the parent of the element under the mouse. Can be combined with {@link
	    ///     #setDebugAll(bool)}
	    /// </summary>
        ...
        {
            if (this._debugParentUnderMouse == debugParentUnderMouse)
                return;

            this._debugParentUnderMouse = debugParentUnderMouse;
	    ...
	    /// <summary>
	    ///     If not {@link TableDebug#none}, debug is enabled only for the first ascendant of the element under the mouse that
	    ///     is a table. Can
	    ...
	    /// <summary>
	    ///     If true, debug is enabled only for the first ascendant of the element under the mouse that is a table. Can be
	    ///     combined with
	    ...
	    /// <summary>
	    ///     sets the gamepad focus element and also turns on gamepad focus for this Stage. For gamepad focus to work you must
	    ///     set an initially
	    ///     focused element.
	    /// </summary>
