...
namespace Nez.Tiled
{
    public class TiledMapReader : ContentTypeReader<TiledMap>
    {
        protected override TiledMap Read(ContentReader reader, TiledMap existingInstance)
        {
            ...
            var tiledMap = new TiledMap(reader.ReadInt32(),
                reader.ReadInt32(),
                reader.ReadInt32(),
                reader.ReadInt32(),
                reader.ReadInt32(),
        ...
        private static TiledLayer ReadTileLayer(ContentReader reader, TiledMap tileMap, string layerName)
        {
            var tileCount = reader.ReadInt32();
                ...
                if (tileId != 0)
                {
                    ...
                    if (tilesetTile != null && tilesetTile.AnimationFrames != null)
                    {
                        if (tilesetTile.AnimationFrames.Count > 0)
                        {
                            ...
                            tileMap.AnimatedTiles.Add(tiles[i] as TiledAnimatedTile);
                        }
                    }
                    else
                    {
        ...
        private static TiledObjectGroup ReadObjectGroup(ContentReader reader, TiledMap tiledMap)
        {
            ...
            for (var i = 0; i < objectCount; i++)
            {
                ...
                if (tiledObjectType == "ellipse")
                {
                    ...
                    obj.ObjType = TiledObject.TiledObjectType.Ellipse;
                }
                else if (tiledObjectType == "image")
                {
                    obj.ObjType = TiledObject.TiledObjectType.Image;
                    throw new NotImplementedException("Image objects are not yet implemented");
                }
                else if (tiledObjectType == "polygon")
                {
                    obj.ObjType = TiledObject.TiledObjectType.Polygon;
                    obj.PolyPoints = ReadVector2Array(reader);
                }
                else if (tiledObjectType == "polyline")
                {
                    obj.ObjType = TiledObject.TiledObjectType.Polyline;
                    obj.PolyPoints = ReadVector2Array(reader);
                }
                else
                {
                    obj.ObjType = TiledObject.TiledObjectType.None;
                }
        ...
        private static Vector2[] ReadVector2Array(ContentReader reader)
        {
