...
namespace Nez.Tiled
{
    public class TiledMap
    {
	    ...
	    public void Update()
        {
            for (var i = 0; i < AnimatedTiles.Count; i++)
        ...
        internal List<TiledAnimatedTile> AnimatedTiles = new List<TiledAnimatedTile>();
        ...
        public TiledTileset CreateTileset(Texture2D texture, int firstId, int tileWidth, int tileHeight,
            bool isStandardTileset, int spacing = 2, int margin = 2, int tileCount = 1, int columns = 1)
        {
	    ...
	    /// <summary>
	    ///     returns the TiledTilesetTile for the given id or null if none exists. TiledTilesetTiles exist only for animated
	    ///     tiles and tiles with
	    ///     properties set.
	    /// </summary>
