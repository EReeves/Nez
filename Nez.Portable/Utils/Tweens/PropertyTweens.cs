...
namespace Nez.Tweens
{
	...
	/// </summary>
	internal class PropertyTarget<T> : ITweenTarget<T> where T : struct
    {
        ...
        public PropertyTarget(object target, string propertyName)
        {
            ...
            {
                Setter = ReflectionUtils.SetterForProperty<Action<T>>(target, propertyName);
                Getter = ReflectionUtils.GetterForProperty<Func<T>>(target, propertyName);
            }

            Assert.IsTrue(Setter != null || _fieldInfo != null,
        ...
        {
            if (_fieldInfo != null)
                return (T) _fieldInfo.GetValue(Target);
            return Getter();
        }
    }
