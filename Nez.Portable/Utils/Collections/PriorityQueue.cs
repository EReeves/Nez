...
namespace Nez.Utils.Collections
{
	...
	public sealed class PriorityQueue<T> : IPriorityQueue<T> where T : PriorityQueueNode
    {
        ...
        public bool Contains(T node)
        {
            ...
            Assert.IsFalse(node.QueueIndex < 0 || node.QueueIndex >= _nodes.Length,
        ...
        {
            Assert.IsNotNull(node, "node cannot be null");
            Assert.IsFalse(Count >= _nodes.Length - 1, "Queue is full - node cannot be added: " + node);
	    ...
	    public T Dequeue()
        {
            Assert.IsFalse(Count <= 0, "Cannot call Dequeue() on an empty queue");
	    ...
	    public T First
        {
            get
            {
                Assert.IsFalse(Count <= 0, "Cannot call .First on an empty queue");
                return _nodes[1];
            }
        }
