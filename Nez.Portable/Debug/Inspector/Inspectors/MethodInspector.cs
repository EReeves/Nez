...
namespace Nez
{
    public class MethodInspector : Inspector
    {
        ...
        {
            var button = new TextButton(Name, skin);
            button.OnClicked += OnButtonClicked;

            // we could have zero or 1 param
            var parameters = (MemberInfo as MethodInfo).GetParameters();
            if (parameters.Length == 0)
            {
                table.Add(button);
                return;
        ...
        private void OnButtonClicked(Button button)
        {
            ...
            {
                (MemberInfo as MethodInfo).Invoke(Target, new object[] { });
            }
            else
            {
                ...
                try
                {
                    ...
                    (MemberInfo as MethodInfo).Invoke(Target, parameters);
                }
                catch (Exception e)
                {
