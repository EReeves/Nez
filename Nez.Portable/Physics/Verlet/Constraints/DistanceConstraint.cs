...
namespace Nez.Physics.Verlet.Constraints
{
	...
	public class DistanceConstraint : Constraint
    {
	    ...
	    /// </summary>
	    private static readonly Polygon Polygon = new Polygon(2, 1);
	    ...
	    /// <summary>
	    ///     sets the tear sensitivity. if the ratio of the current distance / restingDistance is greater than tearSensitivity
	    ///     the
        ...
        public override void HandleCollisions(int collidesWithLayers)
        {
            ...
            var colliders = Physics.BoxcastBroadphase(ref Polygon.Bounds, collidesWithLayers);
            foreach (var collider in colliders)
            {
                CollisionResult result;
                if (Polygon.CollidesWithShape(collider.Shape, out result))
                {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ApproximateCollisionsWithPoints(int collidesWithLayers)
        {
            Vector2 pt;
            for (var j = 0; j < TotalPointsToApproximateCollisionsWith - 1; j++)
            {
                ...
                var collidedCount = Physics.OverlapCircleAll(pt, 3, VerletWorld.Colliders, collidesWithLayers);
                for (var i = 0; i < collidedCount; i++)
                {
                    var collider = VerletWorld.Colliders[i];
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void PreparePolygonForCollisionChecks(out Vector2 midPoint)
        {
            // set our Polygon points
            midPoint = Vector2.Lerp(_particleOne.Position, _particleTwo.Position, 0.5f);
            Polygon.Position = midPoint;
            Polygon.Points[0] = _particleOne.Position - Polygon.Position;
            Polygon.Points[1] = _particleTwo.Position - Polygon.Position;
            Polygon.RecalculateCenterAndEdgeNormals();
        }
