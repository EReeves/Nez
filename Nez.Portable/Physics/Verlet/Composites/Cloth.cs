            ...
            float tearSensitivity = 5, bool connectHorizontalParticles = true)
        {
            ...
            for (var x = 0; x < segments; x++)
            {
                ...
                if (connectHorizontalParticles && x > 0)
                    AddConstraint(new DistanceConstraint(Particles[y * segments + x], Particles[y * segments + x - 1],
                            stiffness))
                        .SetTearSensitivity(tearSensitivity)
                        .SetCollidesWithColliders(false);

                if (y > 0)
                    AddConstraint(new DistanceConstraint(Particles[y * segments + x], Particles[(y - 1) * segments + x],
                            stiffness))
                        .SetTearSensitivity(tearSensitivity)
                        .SetCollidesWithColliders(false);
