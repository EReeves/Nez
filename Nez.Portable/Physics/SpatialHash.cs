...
namespace Nez.Physics
{
    public class SpatialHash
    {
	    ...
	    /// </summary>
	    private readonly IntIntDictionary _cellDict = new IntIntDictionary();
	    ...
	    /// </summary>
	    private readonly Box _overlapTestBox = new Box(0f, 0f);

	    /// <summary>
	    ///     cached circle used for overlap checks
	    /// </summary>
	    private readonly Circle _overlapTestCirce = new Circle(0f);
	    ...
	    /// </summary>
	    private readonly HashSet<Collider> _tempHashset = new HashSet<Collider>();
	    ...
	    /// <summary>
	    ///     gets the cell at the world-space x,y value. If the cell is empty and createCellIfEmpty is true a new cell will be
	    ///     created.
	    /// </summary>
	    ...
	    [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private List<Collider> CellAtPosition(int x, int y, bool createCellIfEmpty = false)
        {
	    ...
	    /// <summary>
	    ///     debug draws the contents of the spatial hash. Note that Core.debugRenderEnabled must be true or nothing will be
	    ///     displayed.
	    /// </summary>
	    ...
	    public HashSet<Collider> AabbBroadphase(ref RectangleF bounds, Collider excludeCollider, int layerMask)
        {
            ...
            for (var y = p1.Y; y <= p2.Y; y++)
            {
                ...
                for (var i = 0; i < cell.Count; i++)
                {
                    ...
                    if (collider == excludeCollider || !layerMask.IsFlagSet(collider.PhysicsLayer))
                        continue;
	...
	///     wraps a Unit32,List
	///     <Collider>
	///         Dictionary. It's main purpose is to hash the int,int x,y coordinates into a single
	///         Uint32 key which hashes perfectly resulting in an O(1) lookup.
	/// </summary>
	internal class IntIntDictionary
    {
        private readonly Dictionary<long, List<Collider>> _store = new Dictionary<long, List<Collider>>();
    ...
    internal class RaycastResultParser
    {
        private static readonly Comparison<RaycastHit> CompareRaycastHits = (a, b) =>
        {
            return a.Distance.CompareTo(b.Distance);
        };

        private readonly List<RaycastHit> _cellHits = new List<RaycastHit>();
        private readonly List<Collider> _checkedColliders = new List<Collider>();
        ...
        public bool CheckRayIntersection(int cellX, int cellY, List<Collider> cell)
        {
            ...
            for (var i = 0; i < cell.Count; i++)
            {
                ...
                if (!_layerMask.IsFlagSet(potential.PhysicsLayer))
                    continue;
