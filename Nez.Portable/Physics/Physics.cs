...
namespace Nez.Physics
{
    public static class Physics
    {
	    ...
	    /// </summary>
	    private static readonly RaycastHit[] HitArray = new RaycastHit[1];
	    ...
	    public static RaycastHit Linecast(Vector2 start, Vector2 end, int layerMask = AllLayers)
        {
            ...
            HitArray[0].Reset();
            LinecastAll(start, end, HitArray, layerMask);
            return HitArray[0];
        }
        ...
        {
            ColliderArray[0] = null;
            _spatialHash.OverlapRectangle(ref rect, ColliderArray, layerMask);
        ...
        {
            Assert.IsFalse(results.Length == 0,
        ...
        {
            ColliderArray[0] = null;
            _spatialHash.OverlapCircle(center, radius, ColliderArray, layerMask);
            return ColliderArray[0];
        }
	    ...
	    /// <summary>
	    ///     updates the colliders position in the physics system. This essentially just removes then re-adds the Collider with
	    ///     its
	    ///     new bounds
	    /// </summary>
	    /// <param name="collider">Collider.</param>
	    ...
	    /// <summary>
	    ///     returns all colliders with bounds that are intersected by collider.bounds. Note that this is a broadphase check so
	    ///     it
	    ...
	    /// <param name="collider">Collider.</param>
	    /// <param name="bounds">Bounds.</param>
	    public static IEnumerable<Collider> BoxcastBroadphaseExcludingSelf(Collider collider, ref RectangleF rect,
	    ...
	    /// <returns>The neighbors excluding self.</returns>
	    /// <param name="collider">Collider.</param>
	    public static IEnumerable<Collider> BoxcastBroadphaseExcludingSelf(Collider collider, float deltaX,
            float deltaY, int layerMask = AllLayers)
        {
