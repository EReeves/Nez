...
namespace Nez.Physics.Shapes
{
    ...
    {
        private bool _areEdgeNormalsDirty = true;
        private Vector2[] edgeNormals;

        // we cache the original details of our polygon
        internal Vector2[] OriginalPoints;

        internal Vector2 PolygonCenter;
	    ...
	    /// <summary>
	    ///     constructs a Polygon from points. points should be specified in clockwise fashion without duplicating the
	    ///     first/last point and
	    ...
	    /// <summary>
	    ///     edge normals are used for SAT collision detection. We cache them to avoid the squareroots. Note that Boxes will
	    ///     only have
	    ...
	    /// <summary>
	    ///     finds the center of the Polygon. Note that this will be accurate for regular polygons. Irregular polygons have no
	    ///     center.
	    /// </summary>
	    ...
	    /// <summary>
	    ///     iterates all the edges of the polygon and gets the closest point on any edge to point. Returns via out the squared
	    ///     distance
	    ///     to the closest point and the normal of the edge it is on. point should be in the space of the Polygon (point -
	    ///     poly.position)
	    /// </summary>
	    ...
	    public static void RotatePolygonVerts(float radians, Vector2[] originalPoints, Vector2[] rotatedPoints)
        {
            ...
            for (var i = 0; i < originalPoints.Length; i++)
            {
                ...
                rotatedPoints[i] =
                    new Vector2(position.X * cos + position.Y * -sin, position.X * sin + position.Y * cos);
            }
        }
	    ...
	    /// <summary>
	    ///     essentially what the algorithm is doing is shooting a ray from point out. If it intersects an odd number of polygon
	    ///     sides
