...
namespace Nez.PhysicsShapes
{
	...
	public static partial class ShapeCollisions
    {
        ...
        private static readonly Vector2[] SatAxisArray = new Vector2[0];
        private static readonly float[] SatTimerPerAxis = new float[0];
        ...
        [Obsolete]
        private static bool PolygonToPolygon(Polygon first, Polygon second, Vector2? deltaMovement,
            out Vector2 responseNormal, out float timeOfCollision)
        {
            ...
            {
                SatAxisArray[iNumAxes] = new Vector2(-deltaMovement.Value.Y, deltaMovement.Value.X);
                var fVel2 = Vector2.Dot(deltaMovement.Value, deltaMovement.Value);
                if (fVel2 > Mathf.Epsilon)
                {
                    if (!IntervalIntersect(first, second, ref SatAxisArray[iNumAxes], ref polygonOffset,
                        ref deltaMovement, out SatTimerPerAxis[iNumAxes]))
                        return false;
                    iNumAxes++;
                }
            }
            ...
            for (int j = first.Points.Length - 1, i = 0; i < first.Points.Length; j = i, i++)
            {
                ...
                SatAxisArray[iNumAxes] = new Vector2(-edge.Y, edge.X);

                if (!IntervalIntersect(first, second, ref SatAxisArray[iNumAxes], ref polygonOffset, ref deltaMovement,
                    out SatTimerPerAxis[iNumAxes]))
                    return false;
                iNumAxes++;
            }
        ...
        private static bool IntervalIntersect(Polygon first, Polygon second, ref Vector2 axis, ref Vector2 shapeOffset,
        ...
        private static void GetInterval(Polygon polygon, int numVertices, Vector2 axis, out float min, out float max)
        {
        ...
        [Obsolete]
        private static bool FindMinimumTranslationDistance(int iNumAxes, out Vector2 normal,
            out float timeOfIntersection)
        {
            ...
            for (var i = 0; i < iNumAxes; i++)
                if (SatTimerPerAxis[i] > 0 && SatTimerPerAxis[i] > timeOfIntersection)
                {
                    mini = i;
                    timeOfIntersection = SatTimerPerAxis[i];
                    normal = SatAxisArray[i];
                    normal.Normalize();
                }
            ...
            for (var i = 0; i < iNumAxes; i++)
            {
                var n = SatAxisArray[i].Length();
                SatAxisArray[i].Normalize();
                SatTimerPerAxis[i] /= n;

                if (SatTimerPerAxis[i] > timeOfIntersection || mini == -1)
                {
                    mini = i;
                    timeOfIntersection = SatTimerPerAxis[i];
                    normal = SatAxisArray[i];
