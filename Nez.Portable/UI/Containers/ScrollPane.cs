...
namespace Nez.UI.Containers
{
	...
	///     The scroll pane's preferred size is that of the child widget. At this size, the child widget will not need to
	///     scroll, so
    ...
    {
        private float _amountX, _amountY;
        private float _areaWidth, _areaHeight;
        private bool _clamp = true;
        protected bool DisableX = true, DisableY;
        private float _fadeAlpha, _fadeAlphaSeconds = 0.5f, _fadeDelay, _fadeDelaySeconds = 1f;
        private bool _fadeScrollBars = true, _smoothScrolling = true;
        private float _flingTime = 1f;
        private float _flingTimer;
        private bool _forceScrollX, _forceScrollY;
        private Rectangle _hKnobBounds;

        private Rectangle _hScrollBounds;
        private bool _hScrollOnBottom = true;
        private float _lastHandlePosition;

        // input data
        private Vector2 _lastMousePos;

        private float _maxX, _maxY;
        private float _overscrollDistance = 50, _overscrollSpeedMin = 30, _overscrollSpeedMax = 200;
        private bool _overscrollX = true, _overscrollY = true;
        private bool _scrollbarsOnTop;

        private float _scrollSpeed = 0.005f;
        private bool _scrollX, _scrollY = true;
        private ScrollPaneStyle _style;
        private bool _touchScrollH, _touchScrollV;
        private bool _useNaturalScrolling = true;
        private bool _variableSizeKnobs = true;

        private float _velocityX, _velocityY;
        private float _visualAmountX, _visualAmountY;
        private Rectangle _vKnobBounds;
        private Rectangle _vScrollBounds;
        private bool _vScrollOnRight = true;
        private Element _widget;
        private Rectangle _widgetAreaBounds;
	    ...
	    /// <summary>
	    ///     Sets the scroll offset so the specified rectangle is fully in view, and optionally centered vertically and/or
	    ///     horizontally,
        ...
        public override void Layout()
        {
            ...
            _scrollX = _forceScrollX || widgetWidth > _areaWidth && !DisableX;
            _scrollY = _forceScrollY || widgetHeight > _areaHeight && !DisableY;
            ...
            if (!fade)
            {
                ...
                if (_scrollY)
                {
                    ...
                    if (!_scrollX && widgetWidth > _areaWidth && !DisableX) _scrollX = true;
                }
                if (_scrollX)
                {
                    _areaHeight -= scrollbarHeight;
                    if (!_scrollY && widgetHeight > _areaHeight && !DisableY)
                    {
                        _scrollY = true;
                        _areaWidth -= scrollbarWidth;
                    }
	    ...
	    /// <summary>
	    ///     Returns the scroll pane's style. Modifying the returned style may not have an effect until {@link
	    ///     #setStyle(ScrollPaneStyle)} is called.
	    /// </summary>
	    ...
	    ///     clipped
	    ///     to the entire scroll pane bounds and the scrollbars are drawn on top of the widget. If {@link
