...
namespace Nez.UI
{
	...
	///     The preferred height of a text field is the height of the {@link TextFieldStyle#font} and {@link
	///     TextFieldStyle#background}.
	///     The preferred width of a text field is 150, a relatively arbitrary size.
	///     The text field will copy the currently selected text when ctrl+c is pressed, and paste any text in the clipboard
	///     when ctrl+v is
	///     pressed. Clipboard functionality is provided via the {@link Clipboard} interface.
	///     The text field allows you to specify an {@link OnscreenKeyboard} for displaying a softkeyboard and piping all key
	///     events
	///     generated by the keyboard to the text field. There are two standard implementations, one for the desktop and one
	///     for Android.
	...
	public class TextField : Element, IInputListener, IKeyboardListener
    {
        ...
        private float _selectionX, _selectionWidth;
        ...
        private int _textHAlign = AlignInternal.Left;
        private int _visibleTextStart, _visibleTextEnd;
        ...
        protected int LetterUnderCursor(float x)
        {
            ...
            x -= TextOffset + FontOffset + halfSpaceSize /*- style.font.getData().cursorX*/ -
                 _glyphPositions[_visibleTextStart];
        ...
        private bool WithinMaxLength(int size)
        {
	    ...
	    /// <summary>
	    ///     When false, text set by {@link #setText(String)} may contain characters not in the font, a space will be displayed
	    ///     instead.
	    ///     When true (the default), characters not in the font are stripped by setText. Characters not in the font are always
	    ///     stripped
	    ...
	    /// <summary>
	    ///     Returns the text field's style. Modifying the returned style may not have an effect until {@link
	    ///     #setStyle(TextFieldStyle)} is called
	    /// </summary>
        ...
        private void Blink()
        {
            if (Time.TotalTime - _lastBlink > _blinkTime)
            {
                _cursorOn = !_cursorOn;
                _lastBlink = Time.TotalTime;
            }
        }
        ...
        {
            if (ShouldIgnoreTextUpdatesWhileFocused && IsFocused)
                return;
	    ...
	    /// <summary>
	    ///     If false, methods that change the text will not fire {@link onTextChanged}, the event will be fired only when user
	    ///     changes the text
	    /// </summary>
        ...
        {
            HasSelection = IsFocused = true;
        }


        void IKeyboardListener.LostFocus()
        {
            HasSelection = IsFocused = false;
            if (_keyRepeatTimer != null)
            {
                _keyRepeatTimer.Stop();
                _keyRepeatTimer = null;
        ...
        public override void Draw(Graphics graphics, float parentAlpha)
        {
            ...
            var fontColor = _disabled && _style.DisabledFontColor.HasValue
                ? _style.DisabledFontColor.Value
                : (IsFocused && _style.FocusedFontColor.HasValue ? _style.FocusedFontColor.Value : _style.FontColor);
            var selection = _style.Selection;
            var background = _disabled && _style.DisabledBackground != null
                ? _style.DisabledBackground
                : (IsFocused && _style.FocusedBackground != null ? _style.FocusedBackground : _style.Background);
            ...
            if (IsFocused && HasSelection && selection != null)
                DrawSelection(selection, graphics, font, x + bgLeftWidth, y + textY + yOffset);

            if (DisplayText.Length == 0)
            {
                if (!IsFocused && _messageText != null)
                {
                    var messageFontColor = _style.MessageFontColor.HasValue
                        ? _style.MessageFontColor.Value
                        : new Color(180, 180, 180, (int) (color.A * parentAlpha));
                    var messageFont = _style.MessageFont != null ? _style.MessageFont : font;
                    graphics.Batcher.DrawString(messageFont, _messageText,
	    ...
	    /// <summary>
	    ///     Copies the selected contents of this TextField to the {@link Clipboard} implementation set on this TextField, then
	    ///     removes it
	    /// </summary>
	    public void Cut()
        {
	    ...
	    /// <summary>
	    ///     Focuses the next TextField. If none is found, the keyboard is hidden. Does nothing if the text field is not in a
	    ///     stage
        ...
        private TextField FindNextTextField(List<Element> elements, TextField best, Vector2 bestCoords,
	    ...
	    /// <summary>
	    ///     Sets the password character for the text field. The character must be present in the {@link BitmapFont}. Default is
	    ///     149 (bullet)
	    /// </summary>
