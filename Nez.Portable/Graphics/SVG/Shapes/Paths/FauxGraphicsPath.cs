...
namespace Nez.Svg
{
	...
	///     It has the full public API that Nez needs for SVG files but the whole mess is all accessed via reflection. It is
	///     slow as all hell and not
	///     recommended for production use. It's only purpose is so that Nez works with SVG files out of the box to get up and
	///     running fast.
	/// </summary>
	public class FauxGraphicsPath
    {
        private readonly object _graphicsPath;


        public FauxGraphicsPath()
        {
            _graphicsPath =
                Activator.CreateInstance(Type.GetType("System.Drawing.Drawing2D.GraphicsPath, System.Drawing"));
        }
