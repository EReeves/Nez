...
namespace Nez
{
	...
	///     - call createAndSetupScene being sure to pass in a renderOrder BEFORE the renderer that contains your reflective
	///     surface.
	...
	///     surfaces should NOT be rendered by the ReflectionRenderer! It needs to create a RenderTexture with just the objects
	///     to reflect.
	///     - you can optionally set a normal map on the ReflectionEffect for a refraction effect
	///     - move the ReflectionRenderer.camera around to get the desired offset for your reflections. You can also change the
	///     zoom of the Camera.
	/// </summary>
	public class ReflectionRenderer : RenderLayerRenderer
    {
        private ReflectionRenderer(int renderOrder, params int[] reflectableObjectRenderLayers) : base(renderOrder,
