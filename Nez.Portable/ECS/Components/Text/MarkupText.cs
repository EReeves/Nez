...
namespace Nez
{
    ...
    {
        private List<ICompiledElement> _compiledMarkup;
        private readonly Dictionary<string, bool> _conditionalDict = new Dictionary<string, bool>();
        private readonly Dictionary<string, IFont> _fontDict = new Dictionary<string, IFont>();

        private readonly Dictionary<string, Texture2D> _textureDict = new Dictionary<string, Texture2D>();
        ...
        public override float Width => TextWidth;
	    ...
	    /// <summary>
	    ///     sets a conditional which can be used in an if tag with a condition attribute. It is also valid to negate the
	    ///     condition by
            ...
            while (reader.Read())
                if (reader.NodeType == XmlNodeType.Element)
                {
                    switch (reader.Name)
                    {
                        ...
                        case "img":
                        {
                            ...
                            if (position.X + imageWidth > TextWidth)
                            {
                                position = WrapLine(position, lineBuffer, alignStack.Peek(), out currentLineHeight);
                                currentTotalHeight += currentLineHeight;
                            }
                ...
                else if (reader.NodeType == XmlNodeType.Text)
                {
                    ...
                    while (words.Length > wordIndex)
                    {
                        ...
                        var currentWordFits = position.X + wordRunSize + wordSize.X <= TextWidth;
        ...
        private Vector2 WrapLine(Vector2 position, List<ICompiledElement> lineBuffer, HorizontalAlign alignment,
            out float currentLineHeight)
        {
            ...
            switch (alignment)
            {
                case HorizontalAlign.Right:
                    xOffset = TextWidth - lineWidth;
                    break;
                case HorizontalAlign.Center:
                    xOffset = (TextWidth - lineWidth) / 2f;
                    break;
            }
        ...
        private static Vector2 ParseVector2(string vectorString)
        {
    ...
    internal interface ICompiledElement
    {
        Vector2 Size { get; }
    ...
    internal struct CompiledTextElement : ICompiledElement
    {
        ...
        private readonly FormatInstruction _formatInstruction;
