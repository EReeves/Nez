...
namespace Nez.ECS.Components.Renderables.Sprites
{
	...
	///     as well if you
	///     prefer). If you do use an Enum it is recommended to pass in a IEqualityComparer when using an enum like CoreEvents
	///     does. See also
    ...
    {
        private readonly Dictionary<TEnum, SpriteAnimation> _animations;
        private int _completedIterations;

        // playback state
        private SpriteAnimation _currentAnimation;

        private TEnum _currentAnimationKey;
	    ...
	    /// <summary>
	    ///     beware the beast man! If you use this constructor you must set the subtexture or set animations so that this sprite
	    ///     has proper bounds
	    ...
	    /// <summary>
	    ///     Sprite needs a Subtexture at constructor time so the first frame of the passed in animation will be used for this
	    ///     constructor
	    /// </summary>
	    ...
	    public SpriteAnimation Play(TEnum animationKey, int startFrame = 0)
        {
            ...
            _totalElapsedTime = startFrame * _currentAnimation.SecondsPerFrame;
            return animation;
        }
