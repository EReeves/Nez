...
namespace Nez.ECS.Components.Renderables
{
	...
	///     Adapted from http://www.paradeofrain.com/2010/01/28/update-on-continuous-2d-trails-in-xna/l
	/// </summary>
	public class LineRenderer : RenderableComponent
    {
        ...
        private float _endWidth = 10;

        // state required for calculating verts and rendering
        private Segment _firstSegment = new Segment();

        private readonly FastList<short> _indices = new FastList<short>(50);
        private readonly Segment _lastSegment = new Segment();
        private float _maxWidth = 10;

        private readonly FastList<SegmentPoint> _points = new FastList<SegmentPoint>();
        private Segment _secondSegment = new Segment();
        private Color _startColor = Color.White;
        private float _startWidth = 10;

        // temporary storage for the texture if it is set before the BasicEffect is created
        private Texture2D _texture;

        private bool _useStartEndColors = true;

        private bool _useStartEndWidths = true;
        private readonly FastList<VertexPositionColorTexture> _vertices = new FastList<VertexPositionColorTexture>(50);
	    ...
	    public EndCapType LineEndCapType { get; protected set; } = EndCapType.Standard;
        ...
        private void AddSingleSegmentLine(ref Segment segment, Color nextPointColor)
        {
            _indices.Add(0);
            _indices.Add(1);
            _indices.Add(2);
	    ...
	    [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void AddFirstSegment(ref Segment segment, ref Segment nextSegment, ref int vertIndex)
        {
            _indices.Add(0);
            _indices.Add(1);
            _indices.Add(4);
	    ...
	    /// <param name="vertIndex">Vert index.</param>
	    [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void AddSegment(ref Segment segment, ref int vertIndex)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void PatchJoint(ref Segment segment, ref int vertIndex)
        {
            switch (LineEndCapType)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void PatchStandardJoint(ref Segment segment, ref int vertIndex)
        {
            if (segment.ShouldFuseBottom)
            {
                _indices.Add((short) vertIndex);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void PatchJaggedJoint(ref Segment segment, ref int vertIndex)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void PatchSmoothJoint(ref Segment segment, ref int vertIndex)
        {
            if (segment.ShouldFuseBottom)
            {
	    ...
	    [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void AddVert(int index, Vector2 position, Vector2 texCoord, Color col)
        {
	    ...
	    /// <summary>
	    ///     sets whether local or world space will be used for rendering. Defaults to world space. Using local space will take
	    ///     into account
        ...
        {
            this.LineEndCapType = endCapType;
            _areVertsDirty = true;
            return this;
        }
	    ...
	    /// <summary>
	    ///     sets the cutoff angle for use with EndCapType.JaggedWithCutoff. Any angles less than the cutoff angle will have
	    ///     jagged
        ...
        {
            this._points.Reset();
            this._points.EnsureCapacity(points.Length);
            for (var i = 0; i < points.Length; i++)
            {
                this._points.Buffer[i].Position = points[i];
                this._points.Length++;
            }
            _areVertsDirty = true;
	    ...
	    /// <summary>
	    ///     adds a point to the line. If start/end widths are not set each point should have a width set here. If start/end
	    ///     colors
        ...
        public LineRenderer AddPoints(Vector2[] points)
        {
            ...
            for (var i = 0; i < points.Length; i++)
            {
                this._points.Buffer[this._points.Length].Position = points[i];
                this._points.Length++;
            }
            _areVertsDirty = true;
        ...
        public override void OnEntityTransformChanged(Transform.Component comp)
        {
            ...
            Bounds.CalculateBounds(Entity.Transform.Position, this.LocalOffset, Vector2.Zero, Entity.Transform.Scale,
