...
namespace Nez
{
    ...
    {
        protected bool ColliderRequiresAutoSizing;

	    /// <summary>
	    ///     flag to keep track of if we registered ourself with the Physics system
	    /// </summary>
	    protected bool IsColliderRegistered;

	    /// <summary>
	    ///     flag to keep track of if our Entity was added to a Scene
	    /// </summary>
	    protected bool IsParentEntityAddedToScene;

        internal bool IsPositionDirty = true;
        internal bool IsRotationDirty = true;
	    
        internal float LocalOffsetLength;
	    ...
	    /// <summary>
	    ///     physicsLayer can be used as a filter when dealing with collisions. The Flags class has methods to assist with
	    ///     bitmasks.
	    /// </summary>
	    public int PhysicsLayer = 1 << 0;

	    /// <summary>
	    ///     the bounds of this Collider when it was registered with the Physics system. Storing this allows us to always be
	    ///     able to
	    ...
	    /// <summary>
	    ///     localOffset is added to entity.position to get the final position for the collider geometry. This allows you to add
	    ///     multiple
	    ...
	    public Vector2 AbsolutePosition => Entity.Transform.Position + LocalOffset;

	    /// <summary>
	    ///     wraps Transform.rotation and returns 0 if this Collider does not rotate with the Entity else it returns
	    ///     Transform.rotation
	    /// </summary>
        ...
        public virtual RectangleF Bounds
        {
            get
            {
                if (IsPositionDirty || IsRotationDirty)
                {
                    ...
                    IsPositionDirty = IsRotationDirty = false;
                }
	    ...
	    /// <summary>
	    ///     localOffset is added to entity.position to get the final position for the collider. This allows you to add multiple
	    ///     Colliders
        ...
        {
            if (this.localOffset != offset)
            {
                UnregisterColliderWithPhysicsSystem();
                this.localOffset = offset;
                LocalOffsetLength = LocalOffset.Length();
	    ...
	    /// <param name="shouldColliderScaleAndRotateWithTransform">
	    ///     If set to <c>true</c> should collider scale and rotate with
	    ///     transform.
	    /// </param>
	    public Collider SetShouldColliderScaleAndRotateWithTransform(bool shouldColliderScaleAndRotateWithTransform)
        {
            ...
            IsPositionDirty = IsRotationDirty = true;
            return this;
        }
        ...
        {
            if (ColliderRequiresAutoSizing)
            {
                // we only deal with boxes and circles here
                Assert.IsTrue(this is BoxCollider || this is CircleCollider,
            ...
            IsParentEntityAddedToScene = true;
            RegisterColliderWithPhysicsSystem();
        }
        ...
        {
            UnregisterColliderWithPhysicsSystem();
            IsParentEntityAddedToScene = false;
        }
	    ...
	    /// <summary>
	    ///     checks to see if this Collider collides with collider. If it does, true will be returned and result will be
	    ///     populated
	    ///     with collision data
	    /// </summary>
	    ...
	    /// <summary>
	    ///     checks to see if this Collider with motion applied (delta movement vector) collides with collider. If it does, true
	    ///     will be
	    ...
	    /// <summary>
	    ///     checks to see if this Collider collides with any other Colliders in the Scene. The first Collider it intersects
	    ///     will have its collision
