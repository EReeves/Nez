...
namespace Nez.Tiled
{
	/// <summary>
	///     WIP
	///     The TiledMapMover is a helper for moving objects around in a gravity-based Tiled map. It requires that the Entity
	///     it is on has a BoxCollider. The
	///     BoxCollider will be used in conjuntion with colliderHorizontal/VerticalInset for all collision detection.
	///     One way platforms can be jumped down through by moving your Transform down 1 pixel and calling
	///     CollisionState.clearLastGroundTile.
	///     If you plan to use slopes/one way platforms with the TiledMapMover some extra properties need to be added to your
	///     tiles in Tiled.
	...
	///     - nez:slopeTopLeft (int): distance in pixels from the tiles top to the slope on the left side. For example, a 45
...
#endif
    {
        ...
        {
            internal TiledTile LastGroundTile;

            // state used by the TiledMapMover
            internal SubpixelFloat MovementRemainderX, MovementRemainderY;
	        ...
	        public void Reset(ref Vector2 motion)
            {
                ...
                MovementRemainderX.Update(ref motion.X);
                MovementRemainderY.Update(ref motion.Y);
                ...
                if (Below && motion.Y == 0 && MovementRemainderY.Remainder > 0)
                {
                    motion.Y = 1;
                    MovementRemainderY.Reset();
                }
            }
	    ...
	    /// </summary>
	    private readonly List<TiledTile> _collidingTiles = new List<TiledTile>();
        ...
        public void TestCollisions(ref Vector2 motion, Rectangle boxColliderBounds, CollisionState collisionState)
        {
            ...
            if (motionX != 0)
            {
                ...
                if (TestMapCollision(sweptBounds, direction, collisionState, out collisionResponse))
                {
                    ...
                    collisionState.MovementRemainderX.Reset();
                }
                else
                {
            ...
            // next, check movement in the vertical dir
            {
                ...
                if (TestMapCollision(sweptBounds, direction, collisionState, out collisionResponse))
                {
                    ...
                    collisionState.MovementRemainderY.Reset();
                ...
                else
                {
                    ...
                    collisionState.LastGroundTile = null;
                }
        ...
        private bool TestMapCollision(Rectangle collisionRect, Edge direction, CollisionState collisionState,
            out int collisionResponse)
        {
            ...
            for (var i = 0; i < _collidingTiles.Count; i++)
            {
                ...
                if (direction.IsHorizontal() && collisionState.LastGroundTile != null &&
                    collisionState.LastGroundTile.IsSlope() && _collidingTiles[i].IsSlope())
                    return false;
                    ...
                    shouldTestSlopes, out collisionResponse))
                {
                    ...
                    if (direction == Edge.Bottom)
                    {
                        collisionState.LastGroundTile = _collidingTiles[i];
                        collisionState.IsGroundedOnOneWayPlatform = collisionState.LastGroundTile.IsOneWayPlatform();
                    }
                ...
                if (collisionState.LastGroundTile != null && direction == Edge.Bottom)
                {
                    ...
                    var isHighSlopeNearest = _collidingTiles[i].GetNearestEdge(perpindicularPosition) ==
                                             _collidingTiles[i].GetHighestSlopeEdge();
                    if (collisionState.LastGroundTile.IsSlope() && _collidingTiles[i].IsSlope() ||
                        !collisionState.LastGroundTile.IsSlope() && isHighSlopeNearest)
                    {
                        // store off our last ground tile if we collided below
                        collisionState.LastGroundTile = _collidingTiles[i];
                        return true;
	    ...
	    private bool TestTileCollision(TiledTile tile, Edge edgeToTest, int perpindicularPosition, int leadingPosition,
	    ...
	    private void PopulateCollidingTiles(Rectangle bounds, Edge direction)
        {
	    ...
	    private int WorldToTilePosition(float worldPosition, Axis axis)
        {
            if (axis == Axis.Y)
